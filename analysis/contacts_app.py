...
from mesh_utils import get_downstream_nns, create_shortest_path_graph, get_links_df_with_locations
...
def generate_mapbox_scatter_data(circle_diameter, color=None):
    ...
    return {
        ...
        'marker': {
            ...
            'line':{
                'color':'white',
                'width':5  # Adjust the outline thickness here
            },
            'opacity':.8
            
        },
        'text': "NN:" + df['nn_string'] + "<br>" + df['nodeName'] + "<br>" + df['Location'],
        'showlegend': False,
    }
...
def create_map_figure():
    ...
    return {
        ...
        'data': [
            ...
            generate_mapbox_scatter_data(marker_diameter+marker_border, color='white'),
...
app.layout = html.Div([
    html.H2("NN Input"),
    dcc.Input(id='nn-input', type='number'),
    ...
    html.Button('Deselect hub', id='regenerate-links', n_clicks=0),
    html.Div(id='marker-click-output'),
    dcc.Textarea(
        id='node-output',
        value=empty_node_text,
...
@app.callback(
    [
        Output('marker-click-output', 'children', allow_duplicate=True), 
...
def deselect_node(n_clicks, current_figure, df_data):
    ...
    return html.H3(""), new_figure, df.to_dict('records'), ''
...
def select_node(click_data, select_node_clicks, current_figure, df_data, nn_input):
    print(click_data)
    ...
    if click_data is not None:
        ...
        selected_nn = new_df.loc[(new_df['Latitude'] == point['lat']) & (new_df['Longitude'] == point['lon'])].iloc[0]["NN"]
    else:
        selected_nn = nn_input

    # update row
    selected_index = new_df.loc[new_df['NN'] == sel ected_nn].index[0]
    row = new_df.iloc[selected_index]
    row['node_state'] = 'selected'
    new_df.iloc[selected_index] = row

    downstream_nns = get_downstream_nns(selected_nn, database_client, shortest_paths, active_nns)
    # downstream_nns_string = ', '.join([str(nn) for nn in downstream_nns])
    downstream_nns_string = nns_to_emails(downstream_nns, database_client)
    ...
    return html.H3(f"NN {row['nn_string']} selected.  Emails:"), new_figure, new_df.to_dict('records'), downstream_nns_string
